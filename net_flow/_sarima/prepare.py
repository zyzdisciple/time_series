...
def prepare_data_with_features(files):
    ...
    temp = []
    for item in result:
        temp.append([int(int(item[-1]) / 1000), item[0]])
    result = temp
    return result
...
def get_hour(timeMillis):
    t = time.localtime(float(timeMillis) / 1000)
    zero_time = time.mktime(time.strptime(time.strftime('%Y-%m-%d %H:00:00', t),'%Y-%m-%d %H:%M:%S'))
