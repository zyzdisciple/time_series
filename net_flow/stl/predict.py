...
def seasonal_decompose_my(time_series):
    res = sm.tsa.seasonal_decompose(time_series.values, freq=336, two_sided=False)
    res.plot()
    plt.show()
    return res
...
def test_stationarity(timeseries):
    ...
    rolmean = timeseries.rolling(window=336).mean()
    rolstd = timeseries.rolling(window=336).std()
...
def expwighted_avg(timeseries):
    #336是指用过去一周的数据来做平滑
    expwighted_avg = timeseries.ewm(halflife=336).mean()
    plt.plot(timeseries)
    plt.plot(expwighted_avg, color='red')
    return expwighted_avg

# test_stationarity(expwighted_avg(src_data()))
# test_stationarity(src_data())
    
#通过 acf(自相关函数) 和 pacf(偏自相关函数) 来确定 pqd.
# p-部分自相关函数表第一次截断的上层置信区间是滞后值。如果你仔细看，该值是p=0
# q-自相关函数表第一次截断的上层置信区间是滞后值。如果你仔细看，该值是q=1
def acf_and_pacf(ts_log_diff):
    #nlags参数 暂时不知道怎么确定.
...
def stl(time_series):
    ...
    seasonal_data = seasonal_data.shift(336)
    ...
    test_score = mean_squared_error(valid[336:], predictions_arima)
    print('Test MSE: %.3f' % test_score)
